...
**Known issue with this dataset:** every single "legitimate" URL in here is a bare homepage with no path at all, not one exception across all ~135,000 of them, while a good chunk of the "phishing" ones do have a real path. That's a collection artifact, not a real pattern, and a model trained on it directly ends up treating "has any path" as a giveaway for phishing (see the README's stage 5 for the full story). `_augment_legitimate_urls_with_paths` in `url_data_loading.py` patches around this by giving half the legitimate URLs a realistic path before training. Worth knowing about if you ever use this raw dataset for something else.
